...
def get_cossims(images, args):
    ...
    with torch.no_grad():
        for i, cl in enumerate(classes):
            ...
            voc_embeds_normed = voc_embeds / np.linalg.norm(voc_embeds, axis=1)[:, np.newaxis]
    ...
    del model
    del processor
    cossims["cossim_mean"] = np.mean(list(cossims.values()))
...
def log_validation(
    ...
    with autocast_ctx:
        ...
        if stable_loss_args is not None:
            trainval_dataloader = stable_loss_args["trainval_dataloader"]
            val_dataloader = stable_loss_args["val_dataloader"]
            vae = stable_loss_args["vae"]
            noise_scheduler = stable_loss_args["noise_scheduler"]
            text_encoder = stable_loss_args["text_encoder"]
            unet = stable_loss_args["unet"]
            weight_dtype = stable_loss_args["weight_dtype"]
            unet.eval()
            with torch.no_grad(), temprngstate(17):

                # Stable Train-Val Loss
                stable_trainval_loss = 0.0
                for step, batch in enumerate(trainval_dataloader):
                    with accelerator.accumulate(unet):
                        ...
                        stable_trainval_loss += avg_loss.item() / args.gradient_accumulation_steps
                ...
                for step, batch in enumerate(val_dataloader):
                    with accelerator.accumulate(unet):
                        ...
                        # Gather the losses across all processes for logging (if we use distributed training).
                        avg_loss = accelerator.gather(loss.repeat(args.train_batch_size)).mean()
                        stable_val_loss += avg_loss.item() / args.gradient_accumulation_steps
            ...
            unet.train()
            # End computation of validation loss
...
def main():
    ...
    # Split into 90% train, 10% train_valid + valid
    train_tvalval = dataset["train"].train_test_split(test_size=0.1)
    # Split the 10% train_valid + valid in half train_valid, half valid
    tvalid_valid = train_tvalval["test"].train_test_split(test_size=0.5)
    # gather everyone if you want to have a single DatasetDict
    dataset = datasets.DatasetDict({
        "train": datasets.concatenate_datasets([train_tvalval["train"], tvalid_valid["train"]]),
        "trainval": tvalid_valid["train"],
        "val": tvalid_valid["test"],
    })
    ...
    with accelerator.main_process_first():
        ...
        trainval_dataset = dataset["trainval"].with_transform(preprocess_train)
    ...
    unet, optimizer, train_dataloader,trainval_dataloader, val_dataloader, lr_scheduler = accelerator.prepare(
        unet, optimizer, train_dataloader, trainval_dataloader, val_dataloader, lr_scheduler
    )
    ...
    # Initializing stable validation loss args
    stable_loss_args = {
        "trainval_dataloader": trainval_dataloader,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if args.checkpointing_epochs is not None:
            if epoch % args.checkpointing_epochs == 0:
                if current_cossim_mean > best_cossim_mean:
                    best_cossim_mean = current_cossim_mean
                    if accelerator.is_main_process:
                        # _before_ saving state, check if this save would set us over the `checkpoints_total_limit`
                        if args.checkpoints_total_limit is not None:
                            checkpoints = os.listdir(args.output_dir)
