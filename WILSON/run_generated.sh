...
if [ $1 = baseline ]; then
  ...
  if [ $2 = disjoint ] || [ $2 = all ]; then
    ...
    exp --name Incr_Gen_Base_Large_RR --step 1 --weakly ${dataset_pars} --alpha 0.5 --lr ${lr} --step_ckpt $pretr --loss_de 1 --lr_policy warmup --affinity --epochs ${epochs} --replay --replay_root ${replay_root} --replay_ratio 0.5
  fi
  ...
  if [ $2 = disjoint ] || [ $2 = all ]; then
    ...
    dataset_pars="--dataset ${dataset} --task ${task} --batch_size 24 $ov --val_interval 2"
    pretr=${path}Base_0.pth
    exp --name Incr_Gen_VOC_RR --step 1 --weakly ${dataset_pars} --alpha 0.5 --lr ${lr} --step_ckpt $pretr --loss_de 1 --lr_policy warmup --affinity --epochs ${epochs} --replay --replay_root ${replay_root}
  fi
  ...
  echo "Invalid argument provided, ./run_generated.sh [baseline/mrte/captions/lora/voc] [overlap/disjoint/all]"
fi